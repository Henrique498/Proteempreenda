(() => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
   ...
   // ── Animação só na primeira visita ou F5 ──────────────────────
  const _initCurtainOriginal = initCurtain;
  function initCurtainComMemoria() {
    if (sessionStorage.getItem('gn_curtain_shown')) return;
    sessionStorage.setItem('gn_curtain_shown', '1');
    _initCurtainOriginal();
  }

  // ── Logo: não recarrega se já estiver na home ──────────────────
  function initLogoClick() {
    const linksParaHome = Array.from(document.querySelectorAll('a[href]')).filter((link) => {
      const href = (link.getAttribute('href') || '').toLowerCase();
      return href === 'index.html' || href === './index.html' || href === '/index.html';
    });

    linksParaHome.forEach((link) => {
      link.addEventListener('click', (e) => {
        ...
        if (localStorage.getItem('gn_token')) {
          localStorage.setItem('gn_returning_home', '1');
        }
      });
    });
  }
   ...
   document.addEventListener('DOMContentLoaded', () => {
    injectCSS();
    initCurtainComMemoria(); // ← era initCurtain()
    initScrollProgress();
    initSectionMaskReveal();
    initHeadingStagger();
    initRevealAnimations();
